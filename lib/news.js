...
export async function getPostData(id) {
  ...
  // replace multiple whitespace with <br>
  contentHtml = addLineBreaks(contentHtml);
...
const addLineBreaks = (content) => {
  ...
  for (let i = 1; i < text.length; i++){
    text[i] = text[i].replace(/\s{2,}/gi, "<br>");
  }
